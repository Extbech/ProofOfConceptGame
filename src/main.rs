...
use player::{tick_cooldown, AttackCooldown, MaxAttackCooldown, Player, PlayerBundle, ProjectileSpeed};
...
fn keyboard_input(
    ...
    mut player: Query<(&mut Transform, &mut Direction, &MovementSpeed, &ProjectileSpeed, &mut AttackCooldown, &MaxAttackCooldown), With<Player>>,
    diagnostics: Res<DiagnosticsStore>,
) {
    let (mut player_trans, mut player_dir, &player_speed, &projectile_speed, mut attack_cooldown, &maxcd) = player.single_mut();
    ...
    (player_position.x, player_position.y) = (player_position.xy() + *player_speed * time.delta_seconds() * keyboard_dir).clamp(-Vec2::splat(BOUND), Vec2::splat(BOUND)).into();
